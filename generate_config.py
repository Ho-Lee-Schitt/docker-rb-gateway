...
for folder in git_repos:
	...
	if bare_repo.bare:
		...
		if new_host is not in known_hosts:
			known_hosts.append(new_host)
			host_data = check_output(["ssh-keyscan", "github.com"])
			kh = open("/root/.ssh/known_hosts", "a")
			kh.write(host_data)
			kh.close()
...
with open('/root/go/src/github.com/reviewboard/rb-gateway/config.json', 'w') as fp:
